...
AdjMatrixGraph::AdjMatrixGraph(int vertices, bool oriented) : Graph(vertices, oriented), matrix(vertices, std::vector<int>(vertices, 0))
{
}
...
bool AdjMatrixGraph::adjacent(int start, int end) const
{
	if (!existsVertex(start) || !existsVertex(end))
	{
		throw std::runtime_error("Ïnvalid vertex");
	}
