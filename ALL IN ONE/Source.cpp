...
struct SNode
{
	...
	SNode* prev;
	SNode* next;
	SNode(std::stack<int> st, SNode* prev = nullptr, SNode* next = nullptr) : st(st), prev(prev),next(next){}
};
struct QNode
{
	std::queue<int> q;
	QNode* prev;
	QNode* next;
	QNode(std::queue<int> q,QNode* prev=nullptr,QNode* next=nullptr) : q(q),prev(prev),next(next){}
};
struct DNode
{
	Node* l;
	DNode* prev;
	DNode* next;
	DNode(Node* data,DNode* prev=nullptr,DNode* next=nullptr) : l(data),prev(prev),next(next){}
};
